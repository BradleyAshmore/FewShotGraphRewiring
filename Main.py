# -*- coding: utf-8 -*-
"""
Created on Tue Jul  9 14:12:43 2024

@author: ashmo

This is the skeleton file. The file will control the main flow of the program.
...
def make_filename(args):
    folder = ".\\Results-FullExperiment-Take2ForCoraOnly"
...
for rep in range(args.repetitions):
    ...
    f = open(fn, 'a')
    f.write(f'Edges Details, Labled Nodes, {args.pair_to_labeled}, All or sampled:,{
            args.add_edges_to}, Pair to syn:, {args.pair_to_synthetic}, \n')
    f.write(f'Threshold: ,{args.threshold}, Oversample multiplier: , {
    ...
    print('Oversampling done. Classifying nodes with oversamples.')
...
#
    ...
    # For full graph
    print('Calculating curve scores.')
    ...
    for epoch in range(args.epochs):
        ...
        data.edge_index = from_networkx(graph_object).edge_index
        print(f'\tEpoch: {epoch:4d} - Curve Score {ending_score:4f} Delta {delta:4f} Score Change {
              real_delta:4f}, edges = {data.edge_index.shape[1]} Num edges {all_curve_scores.size()}, Time: {elapsed:4f}')
